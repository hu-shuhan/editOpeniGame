...
bool ContourFilter::ExecuteWithVolumeMesh(VolumeMesh::Pointer vm) {
    if (!vm) return false;
    if (vm->GetIsPolyhedronType()) { return this->ExecuteWithVolumeMeshWithPolyhedronType(vm); }
...
void ContourFilter::ComputePointValueAndCellVisible(Points::Pointer inPoints, CellArray::Pointer inCells,
                                                    DoubleArray::Pointer PointContourArray,
                                                    CharArray::Pointer CellVisible) {
    igIndex PointId = 0;
    ...
    for (PointId = 0; PointId < inPointNum; PointId++) {
        PointContourValue[PointId] =
                this->m_SelectedScalar->GetElementValue(PointId, m_SeletectDimension) - m_ContourValue;
    }
    igIndex CellId = 0;
...
void ContourFilter::CopyAttributeSetData(igIndex outPointNum, igIndex outCellNum, AttributeSet::Pointer inData,
