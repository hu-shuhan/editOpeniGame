...
enum class IGCLayout {
    Native,
    Legacy32,
    Legacy64,
};

inline constexpr IGCLayout ResolveIGCLayout(IGCLayout layout) {
    if (layout == IGCLayout::Native) { return sizeof(IGenum) == 4 ? IGCLayout::Legacy32 : IGCLayout::Legacy64; }
    return layout;
}
...
template<typename IGenumT>
struct LegacyAttrStorageParams : FloatStorageParams {
    char name[256] = {};        // 名称
    IGenumT type = 0;           // 类型 IG_SCALAR, IG_VECTOR, IG_NORMAL, IG_TCOORD, IG_TENSOR
    IGenumT attachmentType = 0; // 附着类型 IG_POINT, IG_CELL
    IGsize binaryCount = 0;     // 在二进制流中的长度
};
