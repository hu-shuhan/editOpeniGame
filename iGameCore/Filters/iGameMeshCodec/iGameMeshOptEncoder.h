...
public:
    ...
    void SetUpdateProgress(Functor&& functor, Args&&... args) {
        this->m_CallBack = std::bind(std::forward<Functor>(functor),
                          std::forward<Args>(args)..., std::placeholders::_1);
    }
    ...
    void TopoEncoder(PayloadBuffer& payload, std::vector<unsigned int>& cellRemap, const std::vector<unsigned int> pointIdRemap)
    {
        ...
        if (isFixedCellSize)
        {
            this->OptimizeCellVertexCache(
                optCellBuffer.data(), cellRemap.data(),
                remappedCellIdBuffer->RawPointer(),
                cellBufferSize,
                pointCount,
                fixedCellSize
            );
        ...
        else
        {
            ...
            this->OptimizeHybirdCellVertexCache(
                optCellBuffer.data(), optCellOffset.data(), cellRemap.data(),
                remappedCellIdBuffer->RawPointer(),
                cellOffset->RawPointer(),
                cellBufferSize,
                cellOffsetSize,
                pointCount,
                cellCount
...
private:
    ...
    void ParamsInit()
    {
        ...
        assert(this->m_Params.meshType == IG_SURFACE_MESH ||
            ...
            this->m_Params.meshType == IG_UNSTRUCTURED_MESH
        );
        if (this->m_Params.meshType == IG_STRUCTURED_MESH)
        {
            std::memcpy(&this->m_Params.structuredMeshParams.axisSize,
                DynamicCast<StructuredMesh>(this->m_DataObj)->GetDimensionSize(), 
                3 * sizeof(int));
        }
        ...
        this->m_Params.geomParams.quantParam =
    ...
    void ParamsEncoder(PayloadBuffer& payload)
    {
        MeshOptParametersWithoutAttr paramsWoAttr
            = static_cast<MeshOptParametersWithoutAttr>(this->m_Params);
        ...
        if (! this->m_Params.attrParams.empty())
        {
            std::memcpy(payload.data() + staticSize,
                this->m_Params.attrParams.data(), dynamicSize);
        }
    }
                ...
                for (int i = 0, tpCursor = 0; i < valueCount; i+=blockSize, tpCursor++)
                {
                    ...
                    tpResult[tpCursor] = tp->Commit([&](int start, int end) ->void {
                        for (int j = start; j < end; j+=2)
                        {
                            ...
                            uint32_t b = (uint32_t)(end - j == 1 ? 0 : meshopt_quantizeHalf(floatBuffer[j + 1]));
                ...
                for (int i = 0, tpCursor = 0; i < valueCount; i += blockSize, tpCursor++)
                {
                    ...
                    tpResult[tpCursor] = tp->Commit([&](int start, int end) ->void {
                        for (int j = start; j < end; j += 2)
                        {
                            ...
                            uint32_t b = (uint32_t)(end - j == 1 ? 0 : meshopt_quantizeHalf((float)doubleBuffer[j + 1]));
                            int shift = sizeof(unsigned short) * 8;
                            uint32_t group = (a << shift) | b;
                            std::memcpy(
                                &quantiBuffer[(j + 1) / 2],
    ...
    void AttrEncoder(PayloadBuffer& payload, const std::vector<unsigned int>& cellRemap, const std::vector<unsigned int>& pointRemap)
    {
        ...
        size_t threadNum =
                std::max(this->m_Params.attrParams.size() / 2, (size_t) 1);
        
        ThreadPool::parallelFor(0, this->m_Params.attrParams.size(),
    ...
    void RunLengthEncoder(std::vector<unsigned char>& dest, std::vector<uint32_t> source)
    {
        ...
        // 对于 AAAABBBBCCCCCC
        // 输出形如 0 4 8 14
        std::vector<std::vector<int>> threadRusult(maxThreadSize); // 线程结果 按顺序存储了每个线程检测到的多个数据变换点的位置
    ...
    // ---------------------------------------------------------------------
    // 下列与cache优化相关的代码改造自 meshoptimizer(license in thirdparty\meshoptimizer-0.21)
    ...
    float VertexScore(int cachePosition, unsigned int liveCellCount)
    {
        ...
        return this->kVertexScoreTableStrip.cache[1 + cachePosition] + this->kVertexScoreTableStrip.live[liveCellClamped];
    }

    unsigned int GetNextCellDeadEnd(
        unsigned int& inputCursor,
        const unsigned char* emittedFlags,
        size_t cellCount)
    {
        // input order
    ...
    void BuildHybirdCellAdjacency(
        ...
        const unsigned int* sourceBuffer, const unsigned int* sourceOffset,
        size_t bufferSize, size_t pointCount, size_t cellCount,
    ...
    void OptimizeHybirdCellVertexCache(
        unsigned int* destCellBuffer, unsigned int* destCellOffset, unsigned int* destCellRemap,
        const unsigned int* sourceCellBuffer, const unsigned int* sourceCellOffsets,
        size_t bufferSize, size_t offsetCount, size_t pointCount, size_t cellCount)
    {
        meshopt_Allocator allocator;

        // guard for empty meshes
        if (bufferSize == 0 || pointCount == 0)
            return;
        ...
        CellAdjacency adjacency = {};
        this->BuildHybirdCellAdjacency(adjacency,
            sourceCellBuffer, sourceCellOffsets,
            bufferSize, pointCount, cellCount, allocator);
        ...
        for (size_t i = 0; i < pointCount; ++i)
            vertexScores[i] = this->VertexScore(-1, liveCellCount[i]);
    ...
    void OptimizeCellVertexCache(
        unsigned int* destCellBuffer, unsigned int* destCellRemap,
        const unsigned int* sourceCellBuffer,
        size_t bufferSize, size_t pointCount, size_t fixedCellSize)
    {
        ...
        for (size_t i = 0; i < pointCount; ++i)
            vertexScores[i] = this->VertexScore(-1, live_cells[i]);
        ...
        while (currentCell != ~0u)
        {
            ...
            for (size_t j = 0; j < fixedCellSize; j++)
            {
                destCellBuffer[outputCell * fixedCellSize + j]
                    = sourceCellBuffer[currentCell * fixedCellSize + j];
