// This file is adapted from the meshoptimizer library; 
// see meshoptimizer.h for version/license details
// 
// Major change: Added support for polycell, change functions'name for compiling
...
public:
	template <typename T>
	static size_t encodeIndexBuffer(unsigned char* buffer, size_t buffer_size, const T* indices, size_t index_count)
	{
		meshopt_IndexAdapter<T> in(NULL, indices, index_count);

		return innerEncodeIndexBuffer(buffer, buffer_size, in.data, index_count);
	}

	template <typename T>
	static int decodeIndexBuffer(T* destination, size_t index_count, const unsigned char* buffer, size_t buffer_size)
	{
		char index_size_valid[sizeof(T) == 2 || sizeof(T) == 4 ? 1 : -1];
		(void)index_size_valid;

		return innerDecodeIndexBuffer(destination, index_count, sizeof(T), buffer, buffer_size);
	}

	static size_t encodeIndexBufferBound(size_t index_count, size_t vertex_count)
	{
...
private:
	inline static unsigned char kIndexHeader = 0xe0;
	inline static unsigned char kSequenceHeader = 0xd0;

	inline 	static int gEncodeIndexVersion = 1;

	typedef unsigned int VertexFifo[16];
	typedef unsigned int EdgeFifo[16][2];

	inline 	static const unsigned int kTriangleIndexOrder[3][3] = {
		{0, 1, 2},
		{1, 2, 0},
		{2, 0, 1},
	};

	inline 	static const unsigned char kCodeAuxEncodingTable[16] = {
		0x00, 0x76, 0x87, 0x56, 0x67, 0x78, 0xa9, 0x86, 0x65, 0x89, 0x68, 0x98, 0x01, 0x69,
		0, 0, // last two entries aren't used for encoding
	};

	static size_t innerEncodeIndexBuffer(
		...
		size_t index_count)
	{
		...
		// modified -----------------------------------
		writeRotateOrder(data, orders.data(), index_count / 3);
		// --------------------------------------------

		return data - buffer;
	}

	static void encodeIndexVersion(int version)
	{
	...
	static int innerDecodeIndexBuffer(
		void* destination,
		size_t index_count,
		size_t index_size,
		...
		size_t buffer_size)
	{
		...
		// modified ----------------------------------------------
		size_t cell_count = index_count / 3;
		std::vector<unsigned int> orders(cell_count);
		readRotateOrder(buffer + buffer_size - (size_t)ceil(cell_count / 4.), orders.data(), cell_count);
		buffer_size -= (size_t)ceil(cell_count / 4.);
		// -------------------------------------------------------
		...
		for (size_t i = 0; i < index_count; i += 3)
		{
			...
			if (codetri < 0xf0)
			{
				...
				if (fec < fecmax)
				{
					...
					resort_vertex(a, b, c, oa, ob, oc, orders[i / 3]);
					writeTriangle(destination, i, index_size, oa, ob, oc);
					// --------------------------------------------------
	...
	static void writeRotateOrder(unsigned char*& destination, unsigned int* order_index, int cell_count)
	{
		for (int i = 0; i < cell_count; i += 4)
		{
			int group_length = std::min(cell_count - i, 4);
			unsigned char group = 0;
			for (int j = 0; j < group_length; j++)
			{
				group |= (*(order_index + i + j) & 3) << ((3 - j) * 2);
			}
	...
	static void readRotateOrder(const unsigned char* source, unsigned int* order_index, int cell_count)
	{
		for (int i = 0; i < cell_count; i += 4)
		{
            int group_length = std::min(cell_count - i, 4);
			...
			for (int j = 0; j < group_length; j++)
			{
				*order_index++ = ((*source) >> ((3 - j) * 2)) & 3;
			}
			source++;
		}
	}

	static void resort_vertex(const unsigned int a, const unsigned int b, const unsigned int c,
		unsigned int& oa, unsigned int& ob, unsigned int& oc,
		int order_index)
	{
		const unsigned int* order = kTriangleIndexOrder[order_index];
		std::vector<unsigned int> out(3);
		out[order[0]] = a;
		out[order[1]] = b;
		out[order[2]] = c;
