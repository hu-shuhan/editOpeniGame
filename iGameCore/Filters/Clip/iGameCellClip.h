...
static void Clip(Tetra::Pointer cell, double* cellValues, Points::Pointer points, CellArray::Pointer connectivity,
...
static void Clip(Tetra::Pointer cell, double* cellValues, PointFinder::Pointer pointFinder,
...
static void Clip(Polyhedron::Pointer cell, double* cellValues, Points::Pointer points, CellArray::Pointer connectivity,
                 UnsignedIntArray::Pointer types, AttributeSet::Pointer inData, AttributeSet::Pointer outData,
                 igIndex cellId, std::vector<InterpolateEdge>& OriginEdge, std::vector<igIndex>& originCell) {
...
static void Clip(Polyhedron::Pointer cell, double* cellValues, PointFinder::Pointer pointFinder,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell) {
    int i = 0, j = 0, vcnt = cell->GetNumberOfPoints();
    ...
    for (int i = 0; i < fcnt; i++) {
        ...
        if (!isCount) {
            for (j = st; j < ed - 2; j++) {
                ...
                tetvalues[1] = cellValues[j + 1];
                tetvalues[0] = cellValues[j + 2];
                Clip(tetra, tetvalues, pointFinder, connectivity, types, inData, outData, cellId, OriginEdge,
                     originCell);
            }
        }
    }
...
static void Clip(Volume::Pointer cell, double* cellValues, Points::Pointer points, CellArray::Pointer connectivity,
                 UnsignedIntArray::Pointer types, AttributeSet::Pointer inData, AttributeSet::Pointer outData,
                 igIndex cellId, std::vector<InterpolateEdge>& OriginEdge, std::vector<igIndex>& originCell,
                 double* pointValues) {
    Tetra::Pointer tetra = Tetra::New();
    ...
    for (int i = 0; i < tetras.size(); i++) {
        ...
        Clip(tetra, tetvalues, points, connectivity, types, inData, outData, cellId, OriginEdge, originCell, true);
    }
}


static void Clip(Volume::Pointer cell, double* cellValues, PointFinder::Pointer pointFinder,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
    ...
    for (int i = 0; i < tetras.size(); i++) {
        ...
        tetra->m_PointIds = tetras[i]->m_PointIds;
        for (j = 0; j < 4; j++) { tetvalues[j] = pointValues[tetra->m_PointIds->GetId(j)]; }
        Clip(tetra, tetvalues, pointFinder, connectivity, types, inData, outData, cellId, OriginEdge, originCell);
    }
}


static void Clip(QuadraticTetra::Pointer cell, double* cellValues, Points::Pointer points,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell) {
    int i = 0, j = 0, vcnt = cell->GetNumberOfPoints();
...
static void Clip(QuadraticTetra::Pointer cell, double* cellValues, PointFinder::Pointer pointFinder,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell) {
    int i = 0, j = 0, vcnt = cell->GetNumberOfPoints();
...
static void Clip(QuadraticPrism::Pointer cell, double* cellValues, Points::Pointer points,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell) {
    int i = 0, j = 0, vcnt = cell->GetNumberOfPoints();
...
static void Clip(QuadraticPrism::Pointer cell, double* cellValues, PointFinder::Pointer pointFinder,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell) {
    int i = 0, j = 0, vcnt = cell->GetNumberOfPoints();
...
static void Clip(QuadraticPyramid::Pointer cell, double* cellValues, Points::Pointer points,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell) {
    int i = 0, j = 0, vcnt = cell->GetNumberOfPoints();
...
static void Clip(QuadraticPyramid::Pointer cell, double* cellValues, PointFinder::Pointer pointFinder,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell) {
    int i = 0, j = 0, vcnt = cell->GetNumberOfPoints();
...
static void Clip(QuadraticHexahedron::Pointer cell, double* cellValues, Points::Pointer points,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell) {
    int i = 0, j = 0, vcnt = cell->GetNumberOfPoints();
...
static void Clip(QuadraticHexahedron::Pointer cell, double* cellValues, PointFinder::Pointer pointFinder,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell) {
    int i = 0, j = 0, vcnt = cell->GetNumberOfPoints();
...
static void Clip(Triangle::Pointer cell, double* cellValues, Points::Pointer points, CellArray::Pointer connectivity,
                 UnsignedIntArray::Pointer types, AttributeSet::Pointer inData, AttributeSet::Pointer outData,
                 igIndex cellId, std::vector<InterpolateEdge>& OriginEdge, std::vector<igIndex>& originCell,
                 bool isMustClip = false) {
    int MASK[3] = {1, 2, 4};
...
static void Clip(Triangle::Pointer cell, double* cellValues, PointFinder::Pointer pointFinder,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell, bool isMustClip = false) {
    int MASK[3] = {1, 2, 4};
    int i, j, CaseIndex = 0;
    igIndex pId = 0;
    igIndex vhs[IGAME_CELL_MAX_SIZE] = {};
...
static void Clip(Quad::Pointer cell, double* cellValues, Points::Pointer points, CellArray::Pointer connectivity,
                 UnsignedIntArray::Pointer types, AttributeSet::Pointer inData, AttributeSet::Pointer outData,
                 igIndex cellId, std::vector<InterpolateEdge>& OriginEdge, std::vector<igIndex>& originCell) {
    int i = 0, j = 0;
    ...
    for (i = 0; i < nPts - 2; i++) {
        ...
        Clip(triangle, trivalues, points, connectivity, types, inData, outData, cellId, OriginEdge, originCell, true);
    }
...
static void Clip(Quad::Pointer cell, double* cellValues, PointFinder::Pointer pointFinder,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell) {
    int i = 0, j = 0;
    ...
    for (i = 0; i < nPts - 2; i++) {
        ...
        Clip(triangle, trivalues, pointFinder, connectivity, types, inData, outData, cellId, OriginEdge, originCell,
             true);
    }
    return;
    //int MASK[4] = { 1,2,4,8 };
...
static void Clip(Polygon::Pointer cell, double* cellValues, Points::Pointer points, CellArray::Pointer connectivity,
                 UnsignedIntArray::Pointer types, AttributeSet::Pointer inData, AttributeSet::Pointer outData,
                 igIndex cellId, std::vector<InterpolateEdge>& OriginEdge, std::vector<igIndex>& originCell) {
    Triangle::Pointer triangle = Triangle::New();
    double trivalues[3] = {};
...
static void Clip(Polygon::Pointer cell, double* cellValues, PointFinder::Pointer pointFinder,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell) {
    Triangle::Pointer triangle = Triangle::New();
...
static void Clip(QuadraticTriangle::Pointer cell, double* cellValues, Points::Pointer points,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell) {
    Triangle::Pointer triangle = Triangle::New();
...
static void Clip(QuadraticTriangle::Pointer cell, double* cellValues, PointFinder::Pointer pointFinder,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell) {
    Triangle::Pointer triangle = Triangle::New();
...
static void Clip(QuadraticQuad::Pointer cell, double* cellValues, Points::Pointer points,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell) {
    Triangle::Pointer triangle = Triangle::New();
...
static void Clip(QuadraticQuad::Pointer cell, double* cellValues, PointFinder::Pointer pointFinder,
                 CellArray::Pointer connectivity, UnsignedIntArray::Pointer types, AttributeSet::Pointer inData,
                 AttributeSet::Pointer outData, igIndex cellId, std::vector<InterpolateEdge>& OriginEdge,
                 std::vector<igIndex>& originCell) {
    Triangle::Pointer triangle = Triangle::New();
