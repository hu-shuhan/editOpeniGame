#include "iGameCurvatureFilter.h"
IGAME_NAMESPACE_BEGIN
bool CurvatureFilter::Execute() {
...
bool CurvatureFilter::ComputeSurfaceCurvatureCotangent(SurfaceMesh::Pointer surface_Mesh,
    ...
    struct EdgeKeyHash {
        size_t operator()(const EdgeKey& k) const { return (size_t(k.a) << 32) ^ size_t(k.b); }
    };
    ...
    areasPos.reserve(nV);
    for (double a: areas)
        if (a > 0) areasPos.push_back(a);
    double areaFloor = 1e-10;
    if (!areasPos.empty()) {
        std::nth_element(areasPos.begin(), areasPos.begin() + areasPos.size() / 2, areasPos.end());
        double med = areasPos[areasPos.size() / 2];
        areaFloor = std::max(1e-12, med * 1e-4);
    }
    for (int i = 0; i < nV; ++i)
        if (mixArea[i] <= areaFloor) mixArea[i] = areaFloor;
    ...
    for (int i = 0; i < nV; ++i) {
        ...
        double K = (isBoundary[i] ? (onePi - angleSum[i]) : (twoPi - angleSum[i])) / mixArea[i];
        H_values[i] = H;
        K_values[i] = K;
    }
...
SurfaceMesh::Pointer CurvatureFilter::TriangulateSurfaceMesh(SurfaceMesh::Pointer mesh) {
    {
        bool f = true;
        igIndex face[16]{};
...
bool CurvatureFilter::GetPointCurvature(int type, Points::Pointer Points, int PointNum) {
...
std::array<float, 3> CurvatureFilter::GetPosition_volume(Volume* v, int num) {
...
std::array<float, 3> CurvatureFilter::GetPosition_face(Face* f, int num) {
    std::array<float, 3> position = {0.0f, 0.0f, 0.0f};
    for (igIndex idx = 0; idx < num; idx++) {
...
bool CurvatureFilter::GetOtherCurvature(int type, int Num) {
...
std::vector<std::array<float, 3>> CurvatureFilter::GetPointGradient(int type, Points::Pointer Points, int PointNum) {
...
std::vector<std::array<float, 3>> CurvatureFilter::GetOtherGradient(int type, int Num) {
    AttributeSet* attributeSet;
    if (type == 0) attributeSet = surface_Mesh->GetAttributeSet();
    else if (type == 1)
        attributeSet = volume_Mesh->GetAttributeSet();
