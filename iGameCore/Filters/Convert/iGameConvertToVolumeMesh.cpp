...
bool ConvertToVolumeMesh::ExecuteWithUnstructuredMesh(UnstructuredMesh::Pointer um) {
    if (!um) return false;
...
bool ConvertToVolumeMesh::ExecuteWithSurfaceMesh(SurfaceMesh::Pointer sm) {
    if (!sm) return false;
    ...
    if (m_ConvertMethod != IG_EXTRACT_VOLUME_CELL) {
        igDebug("表面网格不支持 " << m_ConvertMethod << " 模式，自动使用 IG_EXTRACT_VOLUME_CELL 模式");
    }
...
void ConvertToVolumeMesh::SetConvertMethod(ConvertMethod CM) {
