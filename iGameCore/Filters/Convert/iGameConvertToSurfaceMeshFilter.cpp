...
ConvertToSurfaceMeshFilter::ConvertToSurfaceMeshFilter() {
...
bool ConvertToSurfaceMeshFilter::Execute() {
...
bool ConvertToSurfaceMeshFilter::ExecuteWithUnstructuredMesh(UnstructuredMesh::Pointer um) {
    if (!um) return false;
...
bool ConvertToSurfaceMeshFilter::ExecuteWithVolumeMesh(VolumeMesh::Pointer vm) {
    if (!vm) return false;
...
bool ConvertToSurfaceMeshFilter::ExecuteWithStructuredMesh(StructuredMesh::Pointer sm) {
    if (!sm) return false;

    SurfaceMesh::Pointer outputMesh = nullptr;

    if (sm->GetDimension() == 3) { return ExecuteWithVolumeMesh(sm); }
    outputMesh = SurfaceMesh::New();
...
void ConvertToSurfaceMeshFilter::SetConvertMethod(ConvertMethod CM) { this->m_ConvertMethod = CM; }
