...
// 编码路径专用的资源参数
struct EncodeResourceBudgetControlParams : public ResourceBudgetControlParams {
    ...
    EncodeResourceBudgetControlParams& SetAttributeScratchQuotaMiB(
        const std::uint64_t valueMiB) noexcept {
        attributeScratchQuotaMiB = valueMiB;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetAttributeMemoryStagingLimitMiB(
        const std::uint64_t valueMiB) noexcept {
        attributeMemoryStagingLimitMiB = valueMiB;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetAttributeManagedStagingLogicalLimitMiB(
        const std::uint64_t valueMiB) noexcept {
        attributeManagedStagingLogicalLimitMiB = valueMiB;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetGeometryEncodeTransferCacheMode(
        const EncodeStorageMode mode) noexcept {
        geometryEncodeTransferCacheMode = mode;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetGeometryEncodeStagingMode(
        const EncodeStorageMode mode) noexcept {
        geometryEncodeStagingMode = mode;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetAttributeEncodeTransferCacheMode(
        const EncodeStorageMode mode) noexcept {
        attributeEncodeTransferCacheMode = mode;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetAttributeEncodeStagingMode(
        const EncodeStorageMode mode) noexcept {
        attributeEncodeStagingMode = mode;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetTopologyEncodeTransferCacheMode(
        const EncodeStorageMode mode) noexcept {
        topologyEncodeTransferCacheMode = mode;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetRemapEncodeStorageMode(
        const EncodeStorageMode mode) noexcept {
        remapEncodeStorageMode = mode;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetPackageFieldStagingMode(
        const EncodeStorageMode mode) noexcept {
        packageFieldStagingMode = mode;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetEncodeReferenceCacheModes(
    ...
    EncodeResourceBudgetControlParams& SetEncodeReferenceResidentLimitMiB(
        const std::uint64_t valueMiB) noexcept {
        encodeReferenceResidentLimitMiB = valueMiB;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetEncodeStorageModes(
    ...
    EncodeResourceBudgetControlParams& SetAttributePressioLanes(
        const std::uint32_t value) noexcept {
        attributePressioLanes = value;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetAttributeReferenceLanes(
        const std::uint32_t value) noexcept {
        attributeReferenceLanes = value;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetRemapMortonLeafMiB(
        const std::uint64_t valueMiB) noexcept {
        remapMortonLeafMiB = valueMiB;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetRemapMortonRunBufferMiB(
        const std::uint64_t valueMiB) noexcept {
        remapMortonRunBufferMiB = valueMiB;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetRemapScratchQuotaMiB(
        const std::uint64_t valueMiB) noexcept {
        remapScratchQuotaMiB = valueMiB;
        return *this;
    }

    EncodeResourceBudgetControlParams& SetRemapBudgetMiB(
...
// 解码路径专用的资源参数
struct DecodeResourceBudgetControlParams : public ResourceBudgetControlParams {
    ...
    DecodeResourceBudgetControlParams& SetAttributeDecodePayloadMode(
        const AttributeDecodePayloadMode mode) noexcept {
        attributeDecodePayloadMode = mode;
        return *this;
    }

    DecodeResourceBudgetControlParams& SetAttributeDecodeCacheMode(
        const DecodeStorageMode mode) noexcept {
        attributeDecodeCacheMode = mode;
        return *this;
    }

    DecodeResourceBudgetControlParams& SetGeometryDecodeCacheMode(
        const DecodeStorageMode mode) noexcept {
        geometryDecodeCacheMode = mode;
        return *this;
    }

    DecodeResourceBudgetControlParams& SetGeometryDecodeReferenceCacheMode(
        const DecodeStorageMode mode) noexcept {
        geometryDecodeReferenceCacheMode = mode;
        return *this;
    }

    DecodeResourceBudgetControlParams& SetTopologyDecodeInputMode(
        const DecodeStorageMode mode) noexcept {
        topologyDecodeInputMode = mode;
        return *this;
    }

    DecodeResourceBudgetControlParams& SetTopologyDecodeCacheMode(
        const DecodeStorageMode mode) noexcept {
        topologyDecodeCacheMode = mode;
        return *this;
    }

    DecodeResourceBudgetControlParams& SetTopologyDecodeReferenceCacheMode(
        const DecodeStorageMode mode) noexcept {
        topologyDecodeReferenceCacheMode = mode;
        return *this;
    }

    DecodeResourceBudgetControlParams& SetAttributeDecodeMemoryPayloadLimitMiB(
        const std::uint64_t valueMiB) noexcept {
        attributeDecodeMemoryPayloadLimitMiB = valueMiB;
        return *this;
    }

    DecodeResourceBudgetControlParams& SetAttributeDecodeMemoryCacheLimitMiB(
        const std::uint64_t valueMiB) noexcept {
        attributeDecodeMemoryCacheLimitMiB = valueMiB;
        return *this;
    }

    DecodeResourceBudgetControlParams& SetGeometryDecodeMemoryCacheLimitMiB(
        const std::uint64_t valueMiB) noexcept {
        geometryDecodeMemoryCacheLimitMiB = valueMiB;
        return *this;
    }

    DecodeResourceBudgetControlParams& SetGeometryDecodeMemoryReferenceLimitMiB(
        const std::uint64_t valueMiB) noexcept {
        geometryDecodeMemoryReferenceLimitMiB = valueMiB;
        return *this;
    }

    DecodeResourceBudgetControlParams& SetTopologyDecodeMemoryInputLimitMiB(
        const std::uint64_t valueMiB) noexcept {
        topologyDecodeMemoryInputLimitMiB = valueMiB;
        return *this;
    }

    DecodeResourceBudgetControlParams& SetTopologyDecodeMemoryCacheLimitMiB(
        const std::uint64_t valueMiB) noexcept {
        topologyDecodeMemoryCacheLimitMiB = valueMiB;
        return *this;
    }

    DecodeResourceBudgetControlParams& SetTopologyDecodeMemoryReferenceLimitMiB(
        const std::uint64_t valueMiB) noexcept {
        topologyDecodeMemoryReferenceLimitMiB = valueMiB;
        return *this;
    }

    DecodeResourceBudgetControlParams& SetDecodeReferenceRetention(
    ...
    DecodeResourceBudgetControlParams& SetAttributeDecodeLanes(
        const std::uint32_t value) noexcept {
        attributeDecodeLanes = value;
        return *this;
    }

    DecodeResourceBudgetControlParams& SetAttributeCommitLanes(
        const std::uint32_t value) noexcept {
...
inline bool ValidateEncodeResourceBudgetControlParams(
    const EncodeResourceBudgetControlParams& params,
    std::string* error = nullptr) {
    if (!ValidateResourceBudgetControlParams(params, error)) {
        return false;
    }
...
inline bool ValidateDecodeResourceBudgetControlParams(
