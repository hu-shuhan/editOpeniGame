...
namespace Ubpa {
	...
	template<typename Traits>
    bool HEMesh<Traits>::Init(const unsigned int* indices, size_t index_count, size_t vertex_count) {
		...
		std::vector<H*> heLoop(3);
		for (size_t i = 0; i < face_count; ++i) {
            for (size_t j = 0; j < 3; j++) {
                size_t next = (j + 1) % 3;
                
                auto* u = vertices[indices[i * 3 + j]];
                auto* v = vertices[indices[i * 3 + next]];
                auto* he = u->HalfEdgeTo(v);
                if (!he) he = AddEdge(u, v)->HalfEdge();
                heLoop[j] = he;
            }
