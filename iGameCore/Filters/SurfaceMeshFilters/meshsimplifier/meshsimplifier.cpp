...
// 仅处理单个内存块的分配和释放，而不涉及数组的内存分配与析构
void* (*Allocator::Storage::allocate)(size_t) = operator new;
void (*Allocator::Storage::deallocate)(void *) = operator delete;
...
static Quadric quadricAdded(const Quadric& L, const Quadric& R) {
...
static void quadricAdded(QuadricGrad* G, const QuadricGrad* R, size_t attribute_count) {
...
static void buildAdjacency(Adjacency& adjacency, const unsigned int* indices, size_t index_count,
                           size_t vertex_count, const unsigned int* remap)
{
    size_t face_count = index_count / 3;
    unsigned int* offsets = adjacency.offsets + 1;
    Adjacency::Edge* data = adjacency.data;
...
static void quadricByTriangle(Quadric& Q, const Vector3f& p0, const Vector3f& p1, const Vector3f& p2, float weight) {
    ...
    float w = normalize(normal);

    float distance = normal.x * p0.x + normal.y * p0.y + normal.z * p0.z;

    // 使用 sqrtf（面积）使误差线性缩放，这往往会改善轮廓
    quadricByPlane(Q, normal.x, normal.y, normal.z, -distance, sqrtf(w) * weight);
}

static void initVertexQuadrics(Quadric* vertex_quadrics, const unsigned int* indices, size_t index_count,
...
static size_t boundEdgeCollapses(const Adjacency& adjacency, size_t vertex_count, size_t index_count) {
    size_t dual_count = 0;
...
static void rankEdgeCollapses(Collapse* collapses, size_t collapse_count, const Vector3f* vertex_positions,
    ...
    for (size_t i = 0; i < collapse_count; ++i) {
        ...
        float ej = quadricError(vertex_quadrics[j0], vertex_positions[j1]);
...
static size_t initEdgeCollapses(Collapse* collapses, size_t collapse_capacity, const unsigned int* indices,
    size_t index_count,
    const float* vertex_attributes, const Vector3f* vertex_positions,
    ...
    for (size_t i = 0; i < index_count; i += 3) {
        ...
        for (int e = 0; e < 3; ++e) {
            ...
            if (attribute_count) {
                c.error +=
                        quadricError(attribute_quadrics[i0], &attribute_gradients[i0 * attribute_count],
                                     attribute_count, vertex_positions[i1], &vertex_attributes[i1 * attribute_count]);
...
static bool hasTriangleFlip(const Vector3f& a, const Vector3f& b, const Vector3f& c, const Vector3f& d) {
    Vector3f eb = {b.x - a.x, b.y - a.y, b.z - a.z};
    Vector3f ec = {c.x - a.x, c.y - a.y, c.z - a.z};
    Vector3f ed = {d.x - a.x, d.y - a.y, d.z - a.z};

    Vector3f nbc = {eb.y * ec.z - eb.z * ec.y, eb.z * ec.x - eb.x * ec.z, eb.x * ec.y - eb.y * ec.x};
    Vector3f nbd = {eb.y * ed.z - eb.z * ed.y, eb.z * ed.x - eb.x * ed.z, eb.x * ed.y - eb.y * ed.x};
...
static bool hasTriangleFlips(const Adjacency& adjacency, const Vector3f* vertex_positions,
                             const unsigned int* collapse_remap, unsigned int i0, unsigned int i1) {
    assert(collapse_remap[i0] == i0);
    assert(collapse_remap[i1] == i1);

    const Vector3f& v0 = vertex_positions[i0];
    const Vector3f& v1 = vertex_positions[i1];


    const Adjacency::Edge* edges = &adjacency.data[adjacency.offsets[i0]];
...
static size_t performEdgeCollapses(unsigned int* collapse_remap, unsigned char* collapse_locked,
                                   ...
                                   const Vector3f* vertex_positions, const Adjacency& adjacency,
...
static void updateQuadrics(const unsigned int* collapse_remap, size_t vertex_count, Quadric* vertex_quadrics,
                           ...
                           const Vector3f* vertex_positions, float& vertex_error) {
    for (size_t i = 0; i < vertex_count; ++i) {
...
size_t simplify_trimesh_with_attriubtes(unsigned int* destination, const unsigned int* indices, size_t index_count,
                                        ...
                                        size_t attribute_count, size_t target_index_count, float target_error,
    ...
    adjacency.data = allocator.allocate<Adjacency::Edge>(index_count);
    ...
    initVertexQuadrics(vertex_quadrics, indices, index_count, vertex_positions);
    ...
    size_t collapse_capacity = boundEdgeCollapses(adjacency, vertex_count, index_count);
    ...
    { 
        ...
        std::cout << "initEdgeCollapses cost time: " << end - start << std::endl;

        if (edge_collapse_count == 0) break;

        //size_t edge_collapse_count = pickEdgeCollapses(edge_collapses, collapse_capacity, result, result_count);
        ...
        //std::cout << "triangle_collapse_goal: " << triangle_collapse_goal << std::endl;
        ...
        std::cout << "performEdgeCollapses cost time: " << end - start << std::endl;
        ...
        std::cout << "remapIndexBuffer cost time: " << end - start << std::endl;
