...
public:
    ...
    igIndex IsInsertedPoint(const Vector3d& x) {
        ...
        if (pId != -1) {
            ...
            if ((x - p).squaredLength() != 0.0) { pId = -1; }
        }
        return pId;
    }
    igIndex InsertUniquePoint(const Vector3d& x) {
        int pId = IsInsertedPoint(x);
        if (pId == -1) {
            pId = m_Points->GetNumberOfPoints();
            IGsize index = this->GetBoxIndex(x);
            auto& box = m_Buffer->GetElement(index);
            if (box == nullptr) {
                box = IdArray::New();
    ...
    IdArray::Pointer GetPointsInBox(igIndex index) const {
        if (index >= 0 && index < m_NumberOfBoxes) { return m_Buffer->GetElement((int) index); }
        return nullptr;
    }
...
protected:
    ...
    void GetOverlappingBoxes(FlexArray<Vector3i>& boxes, const Vector3d x, const Vector3i ijk, double dist) {
        int i, j, k;
        ...
        for (i = minLevel[0]; i <= maxLevel[0]; i++) {
            for (j = minLevel[1]; j <= maxLevel[1]; j++) {
                for (k = minLevel[2]; k <= maxLevel[2]; k++) {
                    if (i < ijk[0] || i > ijk[0] || j < ijk[1] || j > ijk[1] || k < ijk[2] || k > ijk[2]) {
                        nei[0] = i;
                        nei[1] = j;
                        nei[2] = k;
                        boxes.push_back(nei);
    ...
    IGsize GetBoxIndex(const Vector3d& x) const {
        ...
        return static_cast<IGsize>(ijk[0]) + static_cast<IGsize>(ijk[1]) * m_Size +
