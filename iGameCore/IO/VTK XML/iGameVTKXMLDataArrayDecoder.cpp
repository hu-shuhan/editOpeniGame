...
bool DecompressZLib(const ByteBuffer& encoded, ByteBuffer& output, std::string& error) {
    ...
    // VTK writers may encode an exact block-size multiple with a zero
    // lastBlockSize.  In that case the final block is a full block; zero is
    // still invalid for a non-empty payload only when the resulting payload
    // cannot satisfy the size and decompression checks below.
    ...
    if (numBlocks - 1 > (std::numeric_limits<std::size_t>::max() - effectiveLastBlockSize) / blockSize) {
        error = "VTK zlib output size overflow";
        return false;
    }
    const std::size_t expectedTotal = (numBlocks - 1) * blockSize + effectiveLastBlockSize;
    output.clear();
    ...
    for (std::size_t block = 0; block < numBlocks; ++block) {
        ...
        const auto expectedSize = block + 1 == numBlocks ? effectiveLastBlockSize : blockSize;
