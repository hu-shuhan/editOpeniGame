...
static void ReadRawBinaryArray(bool is_header_8_byte, char* p, typename FlatArray<T>::Pointer arr) {
    if (p == nullptr || arr == nullptr) { return; }
    if (arr->GetDimension() <= 0) { return; }
    char* current = p;
    uint64_t byte_size;
    if (is_header_8_byte) {
        byte_size = bytes_to_target<uint64_t>(p);
