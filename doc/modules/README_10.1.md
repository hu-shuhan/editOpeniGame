...
| --- | --- |
| 打开文件 | 打开 `car.vtk` |
| 菜单「算法处理」→「零件分割」 | 在线调用 P3SAM 服务端；弹窗配置服务器 IP / 端口（默认 `127.0.0.1:8765`）；对当前模型三角化并简化后发送 OBJ，接收带 `part_id` 的 VTK 并映射回原始网格 |
| 菜单「算法处理」→「零件分割(自文件)」 | 读取已有的带 `part_id` VTK，将结果映射到当前模型（使用 `output_p600000_pr4000_t0.95_s42_npp.vtk`） |
...
| 项目  | 入口 / 输入 | 说明  |
| --- | --- | --- |
| 在线分割验证 | 「零件分割」 | 先启动 P3SAM 服务端（默认 `127.0.0.1:8765`）；菜单中输入服务器 IP / 端口；当前模型须为可三角化网格（OBJ / VTK / STL 等）；成功后当前模型获得 `part_id` 单元属性 |
...
> 在线分割时，C++ 端默认只保留约 10% 面片发送给服务器，并默认关闭后处理；若分割细节不足，可调整 `P3SAMSegmenter` 的 `SetSimplificationRatio` / `SetPostProcess`，或调高服务端 `--point_num` / `--prompt_num` 后重新编译 / 启动。
> 
