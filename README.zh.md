...
```mermaid
%%{init: {"theme": "base", "flowchart": {"curve": "linear", "htmlLabels": true, "nodeSpacing": 18, "rankSpacing": 42}, "themeVariables": {"fontFamily": "Microsoft YaHei, Noto Sans CJK SC, sans-serif", "primaryTextColor": "#172033", "lineColor": "#d3dce9"}}}%%
flowchart TB
    ...
    subgraph kernel["可视化自主内核"]
        direction TB

        subgraph moduleRow[" "]
            direction LR

            subgraph analysis["智能可视分析模块"]
                direction LR
                mcp(["MCP<br/>服务"])
                intelligent(["智能特征<br/>提取"])
                language(["自然语言<br/>交互"])
                report(["可视化报告<br/>生成"])
            end
    ...
    interfaces <--> kernel
    analysis <--> coreModule
    coreModule <--> rendering
    coreModule <--> filters

    classDef app fill:#adc3e9,stroke:#adc3e9,color:#172033,stroke-width:1px;
    classDef title fill:transparent,stroke:transparent,color:#111827,font-size:28px,font-weight:500;
    classDef interface fill:#f2f6ff,stroke:#d8e2f2,color:#172033,stroke-width:2px;
    classDef module fill:#adc3e9,stroke:#adc3e9,color:#172033,stroke-width:1px;

    class cloud,vector,tensor,deformation,transient,animation app;
    class appTitle title;
    class gui,cppApi interface;
    class mcp,intelligent,language,report,dataModel,memory,algorithms,events,process,scene,interaction,graphics,parallel,fileIO,conversion,interpolation,mesh,field,feature,simplify,selection,imageOutput,chart,advanced module;

    style application fill:#f7f7f7,stroke:#c9c9c9,stroke-width:2px
    style interfaces fill:#f7f7f7,stroke:#c9c9c9,stroke-width:2px
    style kernel fill:#f7f7f7,stroke:#c9c9c9,stroke-width:2px
    style moduleRow fill:transparent,stroke:transparent
    style analysis fill:#f3f6fc,stroke:#d8e2f2,stroke-width:2px
    style coreModule fill:#f3f6fc,stroke:#d8e2f2,stroke-width:2px
    style rendering fill:#f3f6fc,stroke:#d8e2f2,stroke-width:2px
    style filters fill:#f3f6fc,stroke:#d8e2f2,stroke-width:2px

    linkStyle default stroke:#d3dce9,stroke-width:4px;
```
