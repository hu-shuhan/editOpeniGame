...
static const std::string kAttributeName = "RelativeVelocity";
...
int main() {
    auto ccmReader = iGame::CCMReader::New();
    auto scene = iGame::Scene::New();
    std::string filePath = "./Models/SteadyFlowLaminarAndTurbulentInAnSBend1_final.ccm";
    ccmReader->SetFilePath(filePath);
    ccmReader->Execute();
    auto obj = ccmReader->GetOutput(0);
    if (obj == nullptr) {
        std::cout << "Read ERROR!\n";
    } else {
        scene->AddModel(obj);
