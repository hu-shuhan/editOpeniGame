...
void igQtMainWindow::initAllUnDefinedComponents() {
    ...
    SliceDockWidget->setWindowTitle("Õ¯∏Ò«–∏Ó");
    ...
    DeformationDockWidget->setWindowTitle("Ω·ππ–Œ±‰");
...
void igQtMainWindow::initToolbarComponent() {
    ...
    //	"color: #202020;"              // …Ë÷√Œƒ±æ—’…´Œ™«≥∞◊…´
    //	"border: 1px solid #ffffff;"   // …Ë÷√±ﬂøÚ—˘ ΩŒ™ª“…´ µœﬂ±ﬂøÚ
    //	"padding: 5px;"                // …Ë÷√ƒ⁄±ﬂæ‡
    //	"font-size: 16px;"              // …Ë÷√œ¬¿≠≤Àµ•œÓ◊÷ÃÂ¥Û–°Œ™14px
    //	"}"
    //	"QComboBox QAbstractItemView {"
    //	"font-family: Arial;"           // …Ë÷√œ¬¿≠≤Àµ•œÓ◊÷ÃÂŒ™Arial
    //	"color: #404040;"               // …Ë÷√œ¬¿≠≤Àµ•œÓ◊÷ÃÂ—’…´Œ™«≥ª“…´
    //	"}"
    //	"QComboBox::drop-down {"
...
void igQtMainWindow::initAllComponents() {
    ...
    connect(ui->action_compress, &QAction::triggered, this, [&](bool checked) {
        ...
        dialog->setFilterTitle("—πÀı");
        dialog->setFilterDescription("◊¢“‚£∫—πÀı∑Ω Ω»°None ±£¨±Ì æ≤ªΩ¯––¡øªØ£ª—πÀı∑Ω Ω»°Float ±£¨±Ì æΩ¯––∏°µ„ ˝¡øªØ£¨«“¡øªØŒª ˝»°÷µ…˙–ß");
        ...
        int id1 = dialog->addParameter(igQtFilterDialogDockWidget::QT_COMBO_BOX, "µ„◊¯±Í—πÀı∑Ω Ω", defaultValue1);

        int id2 = dialog->addParameter(igQtFilterDialogDockWidget::QT_LINE_EDIT, "µ„◊¯±Í¡øªØŒª ˝", "16");

        int id3 = dialog->addParameter(igQtFilterDialogDockWidget::QT_COMBO_BOX, " Ù–‘—πÀı∑Ω Ω", defaultValue2);

        int id4 = dialog->addParameter(igQtFilterDialogDockWidget::QT_LINE_EDIT, " Ù–‘¡øªØŒª ˝", "16");
        ...
        dialog->setApplyFunctor([=]() {
            ...
            if (ok) {
                QMessageBox::information(this, "", "—πÀı≥…π¶");
                dialog->close();
            } else
                QMessageBox::information(this, "", "—πÀı ß∞‹");
        });
    });
    ...
    connect(ui->action_SaveScreenShot, &QAction::triggered, this, [&]() {
        ...
        if (saved_image.save(path, "BMP")) {
            QMessageBox::information(this, "", "±£¥Ê≥…π¶");
        } else {
            QMessageBox::information(this, "", "±£¥Ê ß∞‹");
        }
    });
...
void igQtMainWindow::initAllFilters() {
    ...
    connect(action_tensorview, &QAction::triggered, this, [&](bool checked) {
        ...
        //ËÆæÁΩÆÂàáÂâ≤ÁöÑÂπ≥Èù¢
        filter->SetPlane(o, n);
        //ËÆæÁΩÆÂàáÂâ≤Ê®°ÂºèÊòØclipËøòÊòØslice
    ...
    QMenu* view = ui->menu_filters->addMenu("Ãÿ’˜Ã·»°");
...
void igQtMainWindow::initAllDockWidgetConnectWithAction() {
    ...
    auto AddClippingMeshToScene = [DrawSurfaceMeshByPointer](const std::string& mainName, SurfaceMesh::Pointer OV,
        ...
        const std::string OVName = "__" + mainName + "_OV";   // ¡Ÿ ±ƒ£–Õ
        const std::string IVName = "__" + mainName + "_IV";   // ¡Ÿ ±ƒ£–Õ
        const std::string OIVName = "__" + mainName + "_OIV"; // ¡Ÿ ±ƒ£–Õ
...
void igQtMainWindow::initAllMySignalConnections() {
    ...
    connect(ui->action_ResetClipping, &QAction::triggered, this, [&](bool checked) {
        ...
        const std::string OVName = "__" + inputMesh->GetName() + "_OV";   // ¡Ÿ ±ƒ£–Õ
        const std::string IVName = "__" + inputMesh->GetName() + "_IV";   // ¡Ÿ ±ƒ£–Õ
        const std::string OIVName = "__" + inputMesh->GetName() + "_OIV"; // ¡Ÿ ±ƒ£–Õ
        bool exist[3]{false, false, false};
        //for (auto& [id, model]: scene->GetModelList()) {
        //    if (model->GetDataObject()->GetName() == OVName) {
        //        auto model = scene->GetModelById(id);
