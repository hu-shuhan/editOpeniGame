...
void igQtSplineOptionDialog::setFileName(const QString& fileName) {
    m_IntroduceLabel->setText(
            QString::asprintf("More than one reader for \" %s \".Please choose one:", fileName.toStdString().c_str()));
}

SplineType igQtSplineOptionDialog::getDialogOutput() { return static_cast<SplineType>(m_ListWidget->currentRow()); }
