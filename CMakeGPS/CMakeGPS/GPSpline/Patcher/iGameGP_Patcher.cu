...
#include "../Kernels/iGameGP_CUDA_Util.cuh"
#include "iGameGP_Patcher.h"
#include "iGameGP_Patcher_Kernel.cuh"
#include "../Util/iGameGP_Log.h"
#include "../Util/iGameGP_Macros.h"
#include "../Util/iGameGP_Timer.h"
...
Patcher::Patcher(
    ...
    const std::unordered_map<std::pair<uint32_t, uint32_t>,
                             uint32_t,
                             ::gpmesh::detail::edge_key_hash,
                             ::gpmesh::detail::edge_key_equal> edges_map,
...
uint32_t Patcher::construct_patches_compressed_format()
{
    ...
    ::cub::DeviceScan::InclusiveSum(m_d_cub_temp_storage_scan,
                                    ...
                                    m_d_patches_offset,
                                    m_num_patches);
    gpmesh::memset<<<blocks_s, threads_s>>>(
...
void Patcher::run_lloyd()
{
    ...
    while (true) {
        ...
        gpmesh::memset<<<blocks_f, threads_f>>>(
            m_d_face_patch, INVALID32, m_num_faces);

        gpmesh::memcpy<<<blocks_s, threads_s>>>(
