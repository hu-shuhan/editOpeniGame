...
struct Report
{
               ...
               bool               append_time_to_file_name = true)
    {
        ...
        if (!ofs.is_open()) {
            GPMESH_ERROR("Report::write() can not open {}", full_name);
        }
    ...
    void model_data(const std::string& model_name, const gpmesh::GPMesh& gpmesh)
    {
        rapidjson::Document subdoc(&m_doc.GetAllocator());
        subdoc.SetObject();

        add_member("model_name", model_name, subdoc);
        add_member("num_vertices", gpmesh.get_num_vertices(), subdoc);
        add_member("num_edges", gpmesh.get_num_edges(), subdoc);
        add_member("num_faces", gpmesh.get_num_faces(), subdoc);
        add_member("max_valence", gpmesh.get_max_valence(), subdoc);
        add_member("is_edge_manifold", gpmesh.is_edge_manifold(), subdoc);
        add_member("is_closed", gpmesh.is_closed(), subdoc);
        add_member("patch_size", gpmesh.get_patch_size(), subdoc);
        add_member("num_patches", gpmesh.get_num_patches(), subdoc);
        add_member("num_components", gpmesh.get_num_components(), subdoc);
        add_member("num_lloyd_run", gpmesh.get_num_lloyd_run(), subdoc);
        add_member("patching_time", gpmesh.get_patching_time(), subdoc);
        uint32_t min_patch_size(0), max_patch_size(0), avg_patch_size(0);
        gpmesh.get_max_min_avg_patch_size(
        ...
        add_member("per_patch_max_vertices",
                   gpmesh.get_per_patch_max_vertices(),
                   subdoc);
        add_member(
            "per_patch_max_edges", gpmesh.get_per_patch_max_edges(), subdoc);
        add_member(
            "per_patch_max_faces", gpmesh.get_per_patch_max_faces(), subdoc);
        add_member("ribbon_overhead (%)", gpmesh.get_ribbon_overhead(), subdoc);
