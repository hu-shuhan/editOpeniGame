                ...
                bool                              quite = false)
{
    ...
    if (NULL == Objfile) {
        GPMESH_ERROR("importOBJ() can not open {}", file_name);
        return false;
    } else {
        if (!quite) {
            GPMESH_TRACE("Reading {}", file_name);
        }
    }
    ...
    while (fgets(line, max_line_length, Objfile) != NULL) {
        ...
        if (sscanf(line, "%s", type) == 1) {
            ...
            if (strcmp(type, "v") == 0) {
                std::istringstream ls {std::string(&line[1])};
                ...
                if ((f.size() > 0 && fn.size() == 0 && ft.size() == 0) ||
                    ...
                    GPMESH_ERROR("importOBJ() face has wrong format Line[{}]",
                                 lineNum);
                    fclose(Objfile);
                    return false;
                }
    ...
    if (!quite) {
        GPMESH_TRACE("import_obj() #vertices= {} ", vertices.size());
        GPMESH_TRACE("import_obj() #faces= {} ", faces.size());
        GPMESH_TRACE("import_obj() #tex= {} ", tex.size());
        GPMESH_TRACE("import_obj() #face_tex= {} ", face_tex.size());
        GPMESH_TRACE("import_obj() #normals = {} ", normals.size());
        GPMESH_TRACE("import_obj() #face_normal= {} ", face_normal.size());
    }
    return true;
}
...
template <typename DataT, typename IndexT>
bool import_obj_Quad(const std::string                 file_name,
                std::vector<std::vector<DataT>>&  vertices,
                std::vector<std::vector<IndexT>>& faces,
                std::vector<std::vector<DataT>>&  tex,
                std::vector<std::vector<IndexT>>& face_tex,
...
template<typename DataT, typename IndexT>
bool import_obj_Quad_VF(const std::string file_name,
                     std::vector<std::vector<DataT>>& vertices,
                     std::vector<std::vector<IndexT>>& faces,
                     bool quite = false)
{
