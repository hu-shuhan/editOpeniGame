...
enum CadSceneFileStatus : int {
    ...
    CADSCENEFILE_ERROR_NOFILE =
            1, // The file did not exist or an I/O operation failed.
    CADSCENEFILE_ERROR_VERSION = 2, // The file had an invalid header.
    CADSCENEFILE_ERROR_OPERATION =
            3, // Called an operation that cannot be called on this object.
...
typedef struct _CSFGeometry {
    ...
    */
    ...
    union {
        ...
        _CSFGeometryAuxChannel* auxStorageOrder = NULL;
    };
    union {
    ...
    union {
        ...
        uint32_t* indexSolid = NULL;
    };

    union {
        CSFoffset indexWireOFFSET;
        uint32_t* indexWire = NULL;
    };

    union {
        CSFoffset partsOFFSET;
        CSFGeometryPart* parts = NULL;
    };

    void release() {
        if (numParts > 0 && parts != NULL) {
            delete[] parts;
            parts = NULL;
        }

        if (numIndexSolid > 0 && indexSolid != NULL) {
            delete[] indexSolid;
            indexSolid = NULL;
        }

        if (numIndexWire > 0 && indexWire != NULL) {
            delete[] indexWire;
            indexWire = NULL;
        }

        if (numVertices > 0 && vertex != NULL) {
            delete[] vertex;
            vertex = NULL;
...
typedef struct _CSFile {
    ...
    void release() {
        for (int id = 0; id < numGeometries; ++id) { geometries[id].release(); }
        ...
        if (numMaterials > 0 && materials != NULL) {
            delete[] materials;
            materials = NULL;
        }
    ...
    union {
        ...
        CSFoffset* pointers = NULL;
    };

    union {
        CSFoffset geometriesOFFSET;
        CSFGeometry* geometries = NULL;
    };

    union {
        CSFoffset materialsOFFSET;
        CSFMaterial* materials = NULL;
    };
...
CSFAPI void* CSFileMemory_alloc(CSFileMemoryPTR mem, size_t sz,
                                const void* fill);
// fillPartial if provided must provide szPartial bytes and szPartial <= sz
CSFAPI void* CSFileMemory_allocPartial(CSFileMemoryPTR mem, size_t sz,
                                       size_t szPartial,
                                       const void* fillPartial);
...
CSFAPI CSFWriteMappingPTR CSFWriteMapping_new(const char* filename,
                                              size_t fileSize);
...
// It must be preserved for as long as the csf and its internals are accessed
CSFAPI int CSFile_load(CSFile** outcsf, const char* filename,
                       CSFileMemoryPTR mem);
...
CSFAPI int CSFile_loadExt(CSFile** outcsf, const char* filename,
                          CSFileMemoryPTR mem);
...
CSFAPI void CSFGeometry_removeAllPartChannels(CSFGeometry* geo,
                                              CSFileMemoryPTR mem);
// if channel doesn't exist, allocates it
// memory wasteful if you do one by one channel at a time
CSFAPI void CSFGeometry_requirePartChannel(CSFGeometry* geo,
                                           CSFileMemoryPTR mem,
                                           CSFGeometryPartChannel channel);
CSFAPI void
CSFGeometry_requirePartChannels(CSFGeometry* geo, CSFileMemoryPTR mem,
                                uint32_t numChannels,
                                const CSFGeometryPartChannel* channels);
CSFAPI void
CSFGeometry_removePartChannels(CSFGeometry* geo, CSFileMemoryPTR mem,
                               uint32_t numChannels,
                               const CSFGeometryPartChannel* channels);
// creates new allocation and copies old
CSFAPI void CSFGeometry_requireAuxChannel(CSFGeometry* geo, CSFileMemoryPTR mem,
                                          CSFGeometryAuxChannel channel);

// add byte packet to meta if its guid doesn't already exist
CSFAPI void CSFMeta_setOrAddBytePacket(CSFMeta** metaPtr,
                                       CSFileMemoryPTR csfmem, size_t size,
                                       const void* data);
...
CSFAPI const float*
CSFGeometry_getNormalChannel(const CSFGeometry* geo,
                             CSFGeometryNormalChannel channel);
// returns vec2*numVertices
CSFAPI const float* CSFGeometry_getTexChannel(const CSFGeometry* geo,
                                              CSFGeometryTexChannel texChannel);
// returns vec4*numVertices
CSFAPI const float* CSFGeometry_getAuxChannel(const CSFGeometry* geo,
                                              CSFGeometryAuxChannel channel);
// returns arbitrary struct array * numParts
CSFAPI const void* CSFGeometry_getPartChannel(const CSFGeometry* geo,
                                              CSFGeometryPartChannel channel);
...
CSFAPI size_t CSFGeometry_getPerPartRequiredSize(const CSFGeometry* geo,
                                                 int numParts);
CSFAPI size_t CSFGeometry_getPerPartRequiredOffset(
        const CSFGeometry* geo, int numParts, CSFGeometryPartChannel channel);
// single element size
CSFAPI size_t CSFGeometryPartChannel_getSize(CSFGeometryPartChannel channel);

// meta can be nullptr
CSFAPI const CSFBytePacket* CSFMeta_getBytePacket(const CSFMeta* meta,
                                                  const CSFGuid guid);

CSFAPI const CSFBytePacket*
CSFMaterial_getBytePacket(const CSFMaterial* material, const CSFGuid guid);
CSFAPI const CSFBytePacket* CSFile_getMaterialBytePacket(const CSFile* csf,
                                                         int materialIDX,
                                                         const CSFGuid guid);

CSFAPI const CSFBytePacket* CSFile_getFileBytePacket(const CSFile* csf,
                                                     const CSFGuid guid);
...
CSFAPI CSFile* CSFileHandle_loadBasics(CSFileHandlePTR handle,
                                       uint32_t contentbitsflag,
                                       CSFileMemoryPTR mem);
...
CSFAPI void* CSFileHandle_loadElements(CSFileHandlePTR handle,
                                       CSFileHandleContentBit contentbit,
                                       int begin, int num, CSFileMemoryPTR mem);

// similar as above but the primary struct array is provided by the pointer, rest of data is allocated into mem
CSFAPI void* CSFileHandle_loadElementsInto(
        CSFileHandlePTR handle, CSFileHandleContentBit contentbit, int begin,
        int num, CSFileMemoryPTR mem, size_t pimarySize, void* primaryArray);
};

#define CSF_logPrintf(outlog, ...)                                             \
    if ((outlog)) { fprintf((outlog), __VA_ARGS__); }

inline size_t CSF_alignIndexAllocation(size_t size) { return (size + 3) & ~3; }

#ifdef __cplusplus

template<typename T>
inline T* CSFGeometry_getPartChannelT(CSFGeometry* geo,
                                      CSFGeometryPartChannel channel) {
    return (T*) CSFGeometry_getPartChannel(geo, channel);
}

template<typename T>
inline const T* CSFGeometry_getPartChannelT(const CSFGeometry* geo,
                                            CSFGeometryPartChannel channel) {
    return (const T*) CSFGeometry_getPartChannel(geo, channel);
}

template<typename T>
inline T* CSFileMemory_allocCopyT(CSFileMemoryPTR mem, const T* src,
                                  size_t num) {
...
template<typename T>
inline T* CSFileMemory_allocPartialT(CSFileMemoryPTR mem, size_t num,
                                     size_t numSrc, const T* src) {
    if (num == 0) return nullptr;
    return (T*) CSFileMemory_allocPartial(mem, sizeof(T) * num,
                                          sizeof(T) * numSrc, src);
}
...
struct CSFReadMapping {
    ...
    CSFReadMapping() {}
    CSFReadMapping(const char* filename) {
        mapping = CSFReadMapping_new(filename);
    }

    CSFReadMapping(CSFReadMapping&& other) noexcept {
    ...
    size_t getSize() const {
        return mapping ? CSFReadMapping_getSize(mapping) : 0;
    }

    const void* getData() const {
        return mapping ? CSFReadMapping_getData(mapping) : nullptr;
    }

    ~CSFReadMapping() {
        if (mapping) { CSFReadMapping_delete(mapping); }
    }
};
...
struct CSFWriteMapping {
    ...
    CSFWriteMapping() {}
    CSFWriteMapping(const char* filename, size_t fileSize) {
        mapping = CSFWriteMapping_new(filename, fileSize);
    }

    CSFWriteMapping(CSFWriteMapping&& other) noexcept {
        this->operator=(std::move(other));
    };
    ...
    size_t getSize() const {
        return mapping ? CSFWriteMapping_getSize(mapping) : 0;
    }

    void* getData() const {
        return mapping ? CSFWriteMapping_getData(mapping) : nullptr;
    }

    ~CSFWriteMapping() {
        if (mapping) { CSFWriteMapping_delete(mapping); }
    }
};
#endif
...
inline uint32_t parallel_items(
        uint64_t numItems,
        std::function<void(uint64_t idx, uint32_t threadIdx, void* userData)>
                fn,
        void* userData = nullptr, uint32_t batchSize = 1,
        uint32_t numThreads = CSF_DEFAULT_NUM_THREADS) {
    if (numThreads <= 1 || numItems < numThreads || numItems < batchSize) {
        for (uint64_t idx = 0; idx < numItems; idx++) { fn(idx, 0, userData); }
        return 1;
    } else {
...
inline uint32_t
parallel_ranges(uint64_t numItems,
                std::function<void(uint64_t idxBegin, uint64_t idxEnd,
                                   uint32_t threadIdx, void* userData)>
                        fn,
                void* userData = nullptr, uint32_t batchSize = 1024,
                uint32_t numThreads = CSF_DEFAULT_NUM_THREADS) {
    if (numThreads <= 1 || numItems < numThreads || numItems < batchSize) {
